...
class USC_dataset(Dataset): 
    ...
    def __read_data__(self):
        ...
        ### 1 : target
        ### 2 : offense
        ### 3 to 62 : x, y, v, goal_angle of players 
        ...
        if self.features == 'S': # univariate precicts univariate
            ...
            if self.use_action_progress :
                # re-ordering array so the target column is last 
                self.data_x = np.concatenate([self.data_x[:,:,1:], self.data_x[:,:,:1]], axis = 2) 
                self.data_y = np.concatenate([self.data_y[:,:,1:], self.data_y[:,:,:1]], axis = 2) 
        ...
        if self.scale:
            ...
            if self.features == 'S' :
                ...
                if self.use_action_progress:
                    self.scaler['mean'].append(train_data[:,-1].mean())
                    self.scaler['scale'].append(train_data[:,-1].std()) 
    ...
    def inverse_transform(self, data):
        for i in range(data.shape[1]):
            data[:,i] = data[:,i] * self.scaler['scale'][i] + self.scaler['mean'][i]
        return data   
    
class USC_dataset_MS(Dataset): 
    ...
    def __read_data__(self):
        ...
        if self.scale:
            ...
            if not self.features == "MS":
                ...
                self.data_y[:,:,0] = (self.data_y[:,:,0] - self.scaler['mean'][-1]) / self.scaler['scale'][-1]
    ...
    def __getitem__(self, index):
        seq_x = self.data_x[index]
        if self.features == 'MS':
            seq_y =  np.concatenate((np.expand_dims(seq_x[-self.args.label_len:,-1],1), self.data_y[index]), axis = 0)
        else :    
            seq_y =  np.concatenate((seq_x[-self.args.label_len:,:], self.data_y[index]), axis = 0)
        # x_mark and y_mark must be of shape [l, 4] and [p+t, 4]
